...
export class CmComAdapter implements TelephonyAdapter {
  ...
  async lookupNumber(e164: string): Promise<NumberLookup> {
    ...
    const json = (await this.get(`${this.baseLookup}/lookup/${encodeURIComponent(parsed)}`)) as {
      valid?: boolean
      countryCode?: string
      country?: string
      lineType?: string
    ...
    const type: NumberLookup['type'] =
      lineType === 'mobile' || lineType === 'landline' || lineType === 'voip' ? lineType : 'unknown'
    return {
  ...
  async verifyWebhookSignature(
    ...
    const clean = signatureHeader.startsWith('sha256=') ? signatureHeader.slice(7) : signatureHeader
...
async function parseCmResponse(res: Response): Promise<unknown> {
  ...
  throw new TelephonyError('provider_error', `CM error ${res.status}: ${text.slice(0, 200)}`, {
    provider: 'cm',
    status: res.status,
  })
}
