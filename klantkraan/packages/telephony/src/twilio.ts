...
export class TwilioAdapter implements TelephonyAdapter {
  ...
  async provisionNumber(opts: ProvisionNumberInput): Promise<{ e164: string }> {
    const parsed = z.object({ country: z.enum(['NL', 'UK']), type: z.literal('local') }).parse(opts)
  ...
  async sendSms(opts: SendSmsInput): Promise<{ messageId: string }> {
    ...
    const json = (await this.form(`${this.base}/2010-04-01/Accounts/${this.sid}/Messages.json`, {
      To: parsed.to,
      From: parsed.from,
      Body: parsed.body,
    })) as { sid?: string }
  ...
  async configureCallForwarding(opts: ConfigureForwardingInput): Promise<void> {
    ...
    await this.form(`${this.base}/2010-04-01/Accounts/${this.sid}/IncomingPhoneNumbers.json`, {
      PhoneNumber: parsed.number,
      VoiceUrl: parsed.targetAgentId,
    })
  }
  ...
  async forwardCall(opts: ForwardCallInput): Promise<{ callId: string }> {
    ...
    const json = (await this.form(`${this.base}/2010-04-01/Accounts/${this.sid}/Calls.json`, {
      To: parsed.to,
      From: parsed.from,
      Twiml: twiml,
    })) as { sid?: string }
  ...
  async verifyWebhookSignature(
    ...
    if (isJson) {
      ...
      signingString = this.webhookUrl + sorted.map(([k, v]) => k + v).join('')
    }
    ...
    return constantTimeEqualBytes(encoder.encode(signatureHeader.trim()), encoder.encode(expected))
  }
...
async function parseTwilioResponse(res: Response): Promise<unknown> {
  ...
  if (code === 21211 || code === 21614 || code === 21610) {
    throw new TelephonyError('invalid_number', `Twilio invalid number (${code})`, {
      provider: 'twilio',
      status: res.status,
    })
  }
  throw new TelephonyError('provider_error', `Twilio error ${res.status}: ${text.slice(0, 200)}`, {
    provider: 'twilio',
    status: res.status,
  })
}
