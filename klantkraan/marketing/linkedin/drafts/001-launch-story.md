---
post_number: 001
publish_week: 1
post_type: launch_story
status: draft
character_count: 1151
---

# Intro — Wat ik bouw en waarom (intern — niet gepubliceerd)

## Voice notes voor founder
- Hook test: "Vandaag launch ik Klantkraan." moet sterk genoeg zijn om de "see more" door te tikken. Eerste regel staat alleen.
- Cadans-controle: korte zinnen, max 1 idee per regel. Witregels tussen blokken voor mobiel.
- Persoonlijk maken: pas "een loodgieter belde mij gisteren" aan naar een echt voorbeeld als die er was.
- Geen emoji's. Geen "AI-gedreven" of "revolutionair". "Doe-maar-gewoon."
- Link in het eerste comment, nooit in de post zelf (LI suppressed link posts).

## Post body (kopieer dit naar LinkedIn)

Vandaag launch ik Klantkraan.

Korte versie: een Nederlandstalige AI-receptionist plus missed-call SMS plus automatische reviews voor loodgieters en dakdekkers.

Waarom dit en waarom nu.

Vakmensen vertellen mij steeds hetzelfde: "Ik mis elke week oproepen. Niet omdat ik lui ben — ik zit onder een keukenkast."

Dat is geen lui-probleem. Dat is een structuurprobleem.

Eén misgelopen badkamer = €1.800 omzet weg. Per week. Per vakman.

Bestaande tools lossen dit half op:
- Voice-only agencies kennen geen voorrijkosten of spoedtoeslag.
- Lead-marketplaces zoals Werkspot? Vakmensen vertrouwen die niet.
- US-tools zoals Podium? 12 maanden lock-in en geen AVG.

Klantkraan is anders:
- Eén vaste prijs, drie modules.
- Per maand opzegbaar, geen setup-kosten.
- EU-hosted, AI Act-disclosure ingebouwd.
- Scripts kennen de vakman-taal.

Voor wie: loodgieters en dakdekkers met 2–8 monteurs, in de Randstad.

Wat ik vraag aan jullie:

Ken jij een loodgieter of dakdekker die structureel oproepen mist? Stuur mij een DM. Ik zoek 2 pilots deze maand — eerste maand 50% korting, daarna gewoon per maand opzegbaar.

#loodgieter #dakdekker #ondernemen

## Eerste comment (kopieer dit als eerste reactie op je eigen post)

Meer info en demo-nummer: klantkraan.nl
