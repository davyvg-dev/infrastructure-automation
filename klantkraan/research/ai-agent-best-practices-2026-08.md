...
*workflows* = LLMs orchestrated through predefined code paths; *agents* = LLMs that
...
2. **Chain-of-Verification** (Meta, 2023): draft → generate verification questions →
   answer them *without seeing the draft* → revise. Hallucinated entities **−77%**
   ...
   *more* than few-shot base models. The verifier must be independent or it copies the
   error.
...
5. **Structured outputs / JSON schema**: eliminates *format* hallucinations (invented
...
- **Reality check**: even frontier models hallucinate on >3-13% of *grounded*
...
1. **A failed tool call is model input, not an exception.** Return errors as
   `tool_result` with `is_error: true` and *instructive* text ("Rate limit exceeded.
...
7. **The cautionary tale**: Project Vend (Anthropic) — an agent running a real shop lost
   ...
   discounts. Verdict: reliability is largely a systems-engineering problem *around* the
...
- Human turn gap: modal **0-200 ms**, median +100 ms; 70-82% of human transitions happen
  ...
  this by planning their reply *while the other person still talks* — the biological
