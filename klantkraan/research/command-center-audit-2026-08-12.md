...
5. **Alerting:** (a) `OnFailure=` on every unit → existing `app.notify.owner` Telegram; (b) dead-man's-switch (healthchecks.io model) optional later. Client-site monitoring: uptime, SSL expiry, DNS/domain expiry, heartbeats. Page only on actionable state *transitions* (watchdog already correct). (pingalert.io, uptimerobot)
