...
| Mobile primary | Personal NL SIM on +316 | Stays on the founder's existing carrier; this is the *founder* number, never published. |
...
| **Linear** | Tasks | Free tier (`07-finance/first-hire-triggers.md`). Single source of truth for *every* commitment. If it's not in Linear, it doesn't exist. |
...
| SOPs | Obsidian + ad-hoc Looms | All SOPs in Notion, every founder action done twice is documented in Notion *before* delegation |
