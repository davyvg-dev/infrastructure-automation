...
- [ ] **Phase 3 — Audit + polish** (serial): migrate the 3 non-page components (DashboardMock/LeadForm/RingingPhone) + delete the 11 deprecated global.css `@theme` kraan tokens so `grep -r -- '--color-kraan' src` is empty (3 `Klantkraan-` brand-copy hits stay); whole-site web-design-guidelines pass; verify a11y, view-transitions, `/demo`, 320px, all locales, price parity, art. 50, `grep -r kraan- src` empty. ■
