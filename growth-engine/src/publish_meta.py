    ...
    *,
    ...
    try:
        ...
        try:
            ...
            detail = body[:300]
        raise MetaPublishError(f"Graph API {exc.code} on {url.split('?')[0]}: {detail}") from None
    except urllib.error.URLError as exc:
        raise MetaPublishError(f"cannot reach {url.split('?')[0]}: {exc.reason}") from None


def _graph(
    path: str, params: dict[str, str], method: str = "POST", timeout: int = 120
) -> dict[str, Any]:
...
def _wait_finished(container_id: str, token: str) -> None:
    ...
    while time.monotonic() < deadline:
        info = _graph(
            container_id, {"fields": "status_code,status", "access_token": token}, method="GET"
        )
...
def verify_auth() -> str:
    ...
    page = _graph(
        page_id, {"fields": "name,instagram_business_account", "access_token": token}, method="GET"
    )
    ...
    note = (
        ""
        if linked == ig_id
        else (f" — WARNING: the Page's linked IG account is {linked!r}, not META_IG_USER_ID")
    )
    return (
        f"Meta OK: Page '{page.get('name')}' ({page_id}), IG @{ig.get('username')} ({ig_id}){note}"
    )
...
def publish_facebook(draft: dict[str, Any], media: dict[str, Any] | None = None) -> str:
    ...
    if media and media.get("path"):
        ...
        if media["type"] == "image":
            body, ctype = _multipart({"message": message, "access_token": token}, "source", path)
            resp = _call(
                f"{_GRAPH}/{_version()}/{page_id}/photos",
                data=body,
                method="POST",
                timeout=300,
...
def _main() -> int:
    ...
    if not (env("META_MEDIA_BASE_URL", required=False) or "").strip():
        print(
            "note: META_MEDIA_BASE_URL unset — IG image posts will fail until "
