...
def test_two_turns_roll_up_into_one_session(data_dir):
    common = dict(client="acme-loodgieter", channel="web", user_id="+31600000000",
                  model="claude-opus-4-8")
    analytics.record_turn(**common, user_text="hi", reply="hello",
                          input_tokens=100, output_tokens=20, tools=[], after_hours=True)
    analytics.record_turn(**common, user_text="book me in", reply="booked!",
                          input_tokens=200, output_tokens=40, after_hours=False,
...
def test_user_id_is_stored_hashed(data_dir):
    analytics.record_turn(client="acme-loodgieter", channel="web", user_id="+31600000000",
                          user_text="hi", reply="hello", input_tokens=1, output_tokens=1,
                          model="claude-opus-4-8", tools=[], after_hours=False)
    sess = analytics.load_session("acme-loodgieter", "web", "+31600000000")
